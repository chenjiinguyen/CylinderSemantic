package CylinderSemantic;

import CylinderSemantic.GUI.CylinderSemanticGUI;

public class Main {
    public static void main(String[] args) {
        /**
         * - Đồ Án Tính Lăng Trụ Tròn Sử Dụng Mạng Ngữ Nghĩa
         * - HUFI - Khoa CNTT - Lớp Trí Tuệ Nhân Tạo + JAVA
         * - Thành Viên:
         * + 2001181078 - Nguyễn Trần Phương Duy - Trưởng Nhóm
         * + 2001181293 - Cao Quang Sơn
         * + 2001181291 - Ngô Thị Như Quỳnh
         * + 2001181437 - Phan Mai Như Ý
         * - Email: duyntp2000@gmail.com
         * - Github: chenjiinguyen
         */
        new CylinderSemanticGUI().setVisible(true);
    }
}
